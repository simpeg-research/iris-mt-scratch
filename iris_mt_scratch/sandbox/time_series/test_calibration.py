...
def parkfield_sanity_check(fft_obj, run_obj, show_response_curves=False,
    ...
    """
    loop over channels in fft obj and make claibrated spectral plots
    ...
    for key in channel_keys:
        ...
        # <FAP RSP>
        if bf4:
            bf4_resp = load_bf4_fap_for_parkfield_test_using_qf_methods(frequencies)
        ...
        # norm_pz_calibration_response = \
        #     channel.channel_response_filter.complex_response(frequencies,
        #                                                      normalize=True)
        # plt.loglog(frequencies, np.abs(norm_pz_calibration_response),
        # label=f"pole-zero {np.max(np.abs(norm_pz_calibration_response)):.2f}")
        # if bf4:
        #     norm_bf4_resp = bf4_resp / np.max(np.abs(bf4_resp))
        #     plt.loglog(frequencies, np.abs(norm_bf4_resp), label=f"fap EMI"
        #     f" {np.max(np.abs(norm_bf4_resp)):.2f}")
        # plt.legend()
        # plt.title(f"Normalized Calibration Response Functions {key}")
        # plt.xlabel("Frequency (Hz)")
        # plt.ylabel("Normalized Response (Hz)")
        # png_name = f"{key}_normalized_response_function.png"
        # plt.savefig(figures_path.joinpath(png_name))
        # plt.show()
        # </CF NORMALIZED RESPONSES>
