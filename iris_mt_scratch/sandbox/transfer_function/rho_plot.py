...
from iris_mt_scratch.sandbox.transfer_function.error_bar_helpers import err_log
...
class RhoPlot(object):
    ...
    def phase_plot(self, pred=None):
        ...
        hfig = figure('Position', [100, 100, 325, 200], 'PaperPosition',[1, 1, 3.25, 2.], 'Tag', 'Phase Plot');
    ...
    def rho_phi_plot(self, pred=None):
        ...
        if pred is not None:
            h_phi = phase_sub_plot(ax, rects.phi, pred);
            h_rho = rho_sub_plot(ax, rects.rho, pred);
        else:
            h_phi = phase_sub_plot(ax, rects.phi);
            h_rho = rho_sub_plot(ax, rects.rho);

        H = {'fig':fig, 'rho': h_rho, 'phi': hphi}
    ...
    def rho_plot(self, **kwargs):
        ...
        rectRho[1] = yMarg;
        hfig = plt.figure('Position', rectScreen, 'PaperPosition', rectPaper,
                       'Tag', 'rho Plot');
        if AddPred:
    ...
    def phase_sub_plot(self, ax, axRect=None, pred=None):
        ...
        [xb, yb] = err_log(np.transpose(self.tf.periods), self.tf.phi[:,0],
                           self.tf.phi_se[:, 0], "XLOG", axis_limits)
        #figure(ax); #need this?
        #set current axes
        #hax = plt.axes('Position', axRect);
        lines = ax.semilogx(xb, yb, 'b-')
        ax.semilogx(self.tf.periods, phi[:,0], 'bo' );
        print("OK, now ser linewidth and markersize")
        #set(lines, 'LineWidth', 1, 'MarkerSize', 7);
        #hold on;
        xb, yb = err_log(np.transpose(self.tf.periods), self.tf.phi[:,1],
                         self.tf.phi_se[:,1],'XLOG', axis_limits)
        lines = ax.semilogx(xb, yb, 'r-')
        ax.semilogx(self.tf.periods, phi[:,1], 'ro');
        #set(lines, 'LineWidth', 1, 'MarkerSize', 7);
        if pred is not None:
            plot(pred.tf.periods, pred.tf.phi[:,0], 'b-', 'linewidth', 2);
            plot(pred.tf.periods, pred.tf.phi[:,1], 'r-', 'linewidth', 2);

        #(lims_ph);
        ax.set_xlim(axis_limits[0], axis_limits[1])
        ax.set_ylim(axis_limits[2], axis_limits[3])
        title_pos_x = np.log(axis_limits[0]) + .1 * (np.log(axis_limits[1] /
                                                       axis_limits[0]));
    ...
    def rho_sub_plot(self, ax, axRect=None, pred=None):
        ...
        xticks = self.get_xticks()
        [xb, yb] = err_log(self.tf.periods ,self.tf.rho[:,0],
                           self.tf.rho_se[:,0], 'XLOG', axis_limits);
        # figure(hfig)
        #
        # hax = axes('Position', axRect);
        error_bars = ax.loglog(xb, yb, 'b-')
        rho_dots = ax.loglog(self.tf.periods ,self.tf.rho[:,0],'bo')
        print("SET LINEWDTHS AND MARKER SIZES")
        #set(lines, 'LineWidth', 1, 'MarkerSize', 7);
        #hold on;
        [xb, yb] = err_log(self.tf.periods, self.tf.rho[:,1],
                           self.tf.rho_se[:,1], 'XLOG', axis_limits);
        lines = ax.loglog(xb, yb, 'r-')
        ax.loglog(self.tf.periods, self.tf.rho[:,1], 'ro');
        print("SET LINEWDTHS AND MARKER SIZES")
        #set(lines, 'LineWidth', 1, 'MarkerSize', 7);
        if pred is not None:
            plot(pred.tf.periods, pred.tf.rho[:,0], 'b-', 'linewidth', 1.5);
            plot(pred.tf.periods, pred.tf.rho[:,1], 'r-', 'linewidth', 1.5);
    ...
    def set_figure_size(self):
        ...
        ydecs = log10(lims[3]) - log10(lims[2])
        ...
        rectPaper =[1., 1., paper_width * paperSizeFac, paper_height * paperSizeFac];

        rectRho =[.15, .15+2.3 / (ydecs+3), .8, ydecs / (ydecs+3) * .8]
        rectPhi =[.15, .15, .8, 2 / (ydecs+3) * .8]
        rects = {'Screen':rectScreen, 'Paper':rectPaper,'Rho':rectRho,
                 'Phi': rectPhi}
        return rects

    def get_xticks(self):
        xticks = 10. ** np.arange(-5, 6)
