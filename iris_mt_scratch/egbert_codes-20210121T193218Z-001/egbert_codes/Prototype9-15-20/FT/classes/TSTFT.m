classdef TSTFT < handle
    ...
    methods
        ...
        function initTS(obj,tsObj)
            ...
            %  need to account for decimation!   Probsbly this
        ...
        function TS2FC(obj,tsObj)
            %   low-pass filters, decimates, windows, FFTs to create FC
            %   objects (one for each decimation level) for input TS in
            %   tsObj   -- could add additional filter steps (e.g.,
            %   prehitening after HP filter, before FT) to this outer level
            ...
            for idec = 1:obj.Dec.NDec
                ...
                obj.FC(idec).FT(tsHP);
                %  transform to physical units  -- here or in FT step?
                %for ich = 1:obj.NCh
                %    obj.FC(idec).FC(ich,:,:) = ...
                %        tsObj.systemResponse(ich).applySystemResponse(obj.FC(idec).FC(ich,:,:),obj.FC(idec).freqs);
                %end
