...
c     -b<badRecFile> read from standard (dnff) bad record file to
C           omit FCs that overlap specified record number segments 
c
...
cnew variables for reordering
      logical         l_GRP, l_SDM, l_PRISET, l_SETS, l_FILE
...
c ME
      ...
      print*,'    zrr (remote reference) are appended to the name'
      print*,'    specified in tranmt.cfg.'
      print*,'    '
      print*,'    Command line options: '
      print*,'      -f<tranmt.cfg> to specify the configuration file'
      ...
      print*,'      -b<badRecFile> read from (dnff) bad record'
      print*,'         record file to omit FCs that overlap specified '
...
ccc      set isuse using info from bad record file
     ...
     &      npts,fac)
         ...
         do i = 1,nuset-1
            do k = 1, nduset
                write(*,*) isuse(2,k,i)+1,' - ',isuse(1,k,i+1)-1,
     &                ' at level ',k
            enddo
         enddo
         do k = 1,nduset
