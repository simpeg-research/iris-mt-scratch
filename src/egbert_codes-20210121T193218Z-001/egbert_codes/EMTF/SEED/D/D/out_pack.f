           ...
           do 30 i=1,ndr
              if(iset(2,i).eq.j) then
                if(iset(3,i).le.iset(4,i)) then
                   ...
                   nf=nf+1
c                  modified by GDE 5/21/2009 ; no rescaling of FCs in case lpack = .false.
                   if(lpack) then
                       do k = 1,nch
                          cout(k,nf) = cwrk(k,i)*1000/  
     &                                         pspecl1(k,j,id-idoff)
                       enddo
                   else
                       do k = 1,nch
                          cout(k,nf) = cwrk(k,i)
...
5              xt1=xt1/10.0
            ix(j,i)=minint
             write(34,*) 'id,ifreq,xt1,i,j = ',id,ifreq,xt1,i,j
            go to 15
6           k1=k-1
            l1=nint(4096*xt1-.5)
               do 10 k=1,8
